...
// A widget created to represent a post in the Posts window.
...
class PostController: public ViewController {
	private:
		// A link to the Box in Glade that will contain all posts from page
		Gtk::Box* posts_container;
		// A link to the Box in Glade that will contain functionality to make a post
		Gtk::Box* make_posts_container;
	...
	public:
		...
		void addPost(Post*);
		// Make a new post and add it to the page
		void makePost(Post*);
